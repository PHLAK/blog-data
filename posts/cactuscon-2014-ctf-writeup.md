...
image:
  url: https://images.unsplash.com/photo-1526374965328-7f61d4dc18c5?ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=1600&h=1000
  caption: Photo by [Markus Spiske](https://unsplash.com/@markusspiske?utm_source=unsplash&amp;utm_medium=referral&amp;utm_content=creditCopyText) on [Unsplash](https://unsplash.com/?utm_source=unsplash&amp;utm_medium=referral&amp;utm_content=creditCopyText)
tags: [Linux, PHP, Hacking, CTF]
---

<!-- excerpt -->
This is a writeup to the 2014 CactusCon web application (SpookiLeaks)
...
actual effect on our ability to read the file. By visiting
...
source of the `/images/manage` page a few things should catch our eye:
  ...
  - There's a hidden field in the image submission form referencing a `file_hash`.
...
This is pretty straightforward. On selection of a file for uploading, a post
...
browser's built-in developer tools we can see the request being made on file
...
properly. Knowing the input and output of this unknown function we can make a
guess as to what it's doing internally and exploit that. In this case, a good
guess is that they are replacing all instances of `../` with nothing. We can
test that out by submitting `....//`. If we're right the inner `../` will be
replaced with nothing leaving a single `../` in its place. Let's test that:
...
user with permission (root perhaps?).
...
of the application as well as any other files on the server with lax permissions.
...
This, along with the contents of `/etc/group` lets us know the "ghost" account
...
we'll stick to the simpler C99 shell) and generate a valid hash for
uploading:
...
Firebug or the browser's built-in web developer tools modify the value of the
...
From here accessing any aspect of the web application or its database is
...
looks like a basic md5. We can verify that by checking out the source of that
custom password hasher at `/images/media?file=....//....//Controller/Component/Auth/CustomPasswordHasher.php`:
...
We now need to be able to log in to the system. Without physical access to
...
remaining now is whether or not the `ghost` user was smart enough to use
